...
class _AddLedgerEntryScreenState extends State<AddLedgerEntryScreen> {
  ...
  static const _modes = ['Cash','UPI','Card','RTGS/NEFT','Bank Transfer','Cheque','Online'];
  static const _categories = ['','Sales','Purchase',...kExpenseCategories];

  @override
  void dispose() {
    _partyCtrl.dispose(); _amountCtrl.dispose();
    _notesCtrl.dispose(); _dateCtrl.dispose();
    super.dispose();
  }
  ...
  void _submit() {
    ...
    if (party.isEmpty || amount == 0) {
      Get.snackbar('Error', 'Party and amount are required',
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ── Type toggle ──
          _card('Transaction Type', Icons.swap_horiz_rounded, Column(children: [
            Row(children: [
              Expanded(child: _typeBtn('CR', 'Credit (CR)', AppColors.success)),
              const SizedBox(width: 10),
              Expanded(child: _typeBtn('DR', 'Debit (DR)', AppColors.error)),
            ]),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: (_type == 'CR' ? AppColors.success : AppColors.error).withOpacity(0.08),
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: (_type == 'CR' ? AppColors.success : AppColors.error).withOpacity(0.3)),
              ),
              child: Row(children: [
                Icon(
                  _type == 'CR' ? Icons.arrow_downward_rounded : Icons.arrow_upward_rounded,
                  color: _type == 'CR' ? AppColors.success : AppColors.error, size: 16,
                ),
          ...
          // ── Entry details ──
          _card('Entry Details', Icons.edit_note_outlined, Column(children: [
            _field(_partyCtrl, 'Party / Description *',
                hint: 'Customer name or expense description'),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _field(_amountCtrl, 'Amount (₹) *',
          ...
          // ── Category & Payment ──
          _card('Category & Payment', Icons.category_outlined, Column(children: [
            _dropdown('Category', _categories,
                _category.isEmpty ? '' : _category,
                (v) => setState(() => _category = v ?? ''),
                hintText: 'Select Category'),
            const SizedBox(height: 10),
            _dropdown('Payment Mode', _modes, _mode,
          ...
          // ── Notes ──
          _card('Notes (Optional)', Icons.notes_outlined,
            Container(
              decoration: BoxDecoration(
            ...
            Expanded(
              ...
              child: ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(
                    backgroundColor: _type == 'CR' ? AppColors.success : AppColors.error,
                    padding: const EdgeInsets.symmetric(vertical: 14),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                    elevation: 0),
              ),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GestureDetector(
        onTap: () async {
          ...
          if (d != null) setState(() => _dateCtrl.text = d.toIso8601String().substring(0, 10));
        },
        child: Container(
