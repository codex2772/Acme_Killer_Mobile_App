...
// ════════════════════════════════════════════════════════════════
// EDIT STAFF SCREEN — mirrors Electron's renderEditStaff()
// ════════════════════════════════════════════════════════════════
...
class _EditStaffScreenState extends State<EditStaffScreen> {
  ...
  Future<void> _submit() async {
    ...
    // ── Try backend API (mirrors Electron's window.jewelERP.staff.update) ──
    bool savedToBackend = false;
    if (_ctrl.isOnline.value && _staff.backendId != null) {
      // Re-fetch updated staff for API payload
      final updated = _ctrl.staffMembers.firstWhereOrNull((s) => s.id == _staffId);
      if (updated != null) {
        final result = await _ctrl.updateViaApi(updated);
        savedToBackend = result.success;
      }
    }
    ...
    Get.snackbar(
      'Saved',
      savedToBackend
          ? '"${_nameCtrl.text.trim()}" saved to database!'
          : '"${_nameCtrl.text.trim()}" updated locally!',
  ...
  Widget build(BuildContext context) {
    if (_staff == null) return const Scaffold(body: Center(child: CircularProgressIndicator()));
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        _sectionCard('Personal Info', Icons.person_outline,
          Column(children: [
            ...
            _row2(
              _tf(_emailCtrl, 'Email', keyboardType: TextInputType.emailAddress),
              _dd('Status', ['Active', 'Inactive'], _status, (v) => setState(() => _status = v!)),
            ),
          ]),
        ),
        ...
        _sectionCard('Compensation', Icons.currency_rupee_rounded, _row3(
          ...
          _tf(_commCtrl,   'Commission (%)',    keyboardType: const TextInputType.numberWithOptions(decimal: true)),
  ...
  Widget _permissionsSection() {
    ...
    return Column(key: const ValueKey('staff_edit'), children: groups.keys.map((group) {
  ...
  Widget _tf(TextEditingController ctrl, String label, {TextInputType? keyboardType}) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
