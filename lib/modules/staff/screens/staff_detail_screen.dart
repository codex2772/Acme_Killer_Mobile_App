...
class StaffDetailScreen extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        body: Column(children: [
              ...
              Expanded(
                child: Column(
                  ...
                  Row(children: [
                    ...
                    _metaTag(Icons.calendar_today_outlined,
          ...
          Padding(
            ...
            child: Row(children: [
              _stat('₹${(staff.salary / 1000).toStringAsFixed(0)}K',
                  'Monthly Salary', AppColors.goldPrimary),
              _stat('$pct%', 'Target Achieved', AppColors.success),
              _stat(ctrl.fmt(commEarned),
                  'Commission (${staff.commission}%)', AppColors.info),
              _stat(
                  '${(staff.leaves['balance'] ?? 0)}',
                  'Leave Balance',
          ...
          Expanded(
            child: TabBarView(children: [
              _OverviewTab(staff: staff, ctrl: ctrl),
  ...
  static Widget _metaTag(IconData icon, String text) =>
...
class _OverviewTab extends StatelessWidget {
  ...
  const _OverviewTab({required this.staff, required this.ctrl});
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      // Permissions card
      _card('Permissions (${staff.permissions.length})',
          ...
          Column(
              ...
              children: groups.keys.map((group) {
                ...
                final granted = perms
                    .where((p) => staff.permissions.contains(p.id))
                    .toList();
      ...
      _card('Recent Attendance', Icons.schedule_outlined,
          Column(children: [
            ...
            ...records.map((a) {
              ...
              Color sc;
              switch (status) {
                case 'Present':  sc = AppColors.success; break;
                case 'Leave':    sc = AppColors.warning; break;
                case 'Sunday':   sc = AppColors.textMuted; break;
                default:         sc = AppColors.error;
              }
              return Container(
                padding: const EdgeInsets.symmetric(
                ...
                child: Row(children: [
                  Expanded(
                      flex: 2,
                      child: Text(
                          _fmtDate(a.date as String),
                          style: const TextStyle(
                              color: AppColors.textSecondary,
                              fontSize: 12))),
                  Expanded(
                      child: Text(
                          (a.clockIn as String).isEmpty
                              ? '—'
                              : a.clockIn as String,
                          style: const TextStyle(
                              color: AppColors.textPrimary,
                  ...
                  Expanded(
                      child: Text(
                          (a.clockOut as String).isEmpty
                              ? '—'
                              : a.clockOut as String,
                          style: const TextStyle(
                              color: AppColors.textPrimary,
                              fontSize: 12))),
                  Expanded(
...
class _PerformanceTab extends StatelessWidget {
  ...
  const _PerformanceTab(
      {required this.staff, required this.ctrl,
       required this.pct, required this.commEarned});
      ...
      _card('Sales Performance', Icons.trending_up_rounded,
          Column(children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
              ...
              Text(
                  '${ctrl.fmt(staff.currentSales)} / ${ctrl.fmt(staff.salesTarget)}',
                  style: const TextStyle(
                      color: AppColors.textPrimary,
                      fontWeight: FontWeight.w500,
                      fontSize: 13)),
...
class _LeavesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      // Leave stats
      Row(children: [
        _leaveStat('${leaves['total']}',   'Total',   AppColors.info),
        _leaveStat('${leaves['used']}',    'Used',    AppColors.error),
        _leaveStat('${leaves['pending']}', 'Pending', AppColors.warning),
        _leaveStat('${leaves['balance']}', 'Balance', AppColors.success),
      ]),
      const SizedBox(height: 14),
      _card('Leave Summary', Icons.event_note_outlined,
          Column(children: [
            _row('Total Leaves',   '${leaves['total']}'),
            _row('Leaves Used',    '${leaves['used']}'),
            _row('Pending Approval','${leaves['pending']}'),
            _row('Balance Leaves', '${leaves['balance']}'),
          ])),
      const SizedBox(height: 12),
      // Leave progress bar
      _card('Leave Utilization', Icons.pie_chart_outline,
          Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
              Text(
                  '${leaves['used']} used of ${leaves['total']}',
                  style: const TextStyle(
                      color: AppColors.textSecondary,
                      fontSize: 13)),
              Text(
                  '${((leaves['used'] as int) * 100 / (leaves['total'] as int)).round()}%',
                  style: const TextStyle(
