        ...
        actions: [
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              icon: const Icon(Icons.person_add_outlined, size: 14, color: Colors.black),
      ...
      body: Column(
        children: [
          ...
          Obx(() => Padding(
            ...
            child: Row(children: [
              _chip('${controller.totalStaff}',  'Total',  AppColors.goldPrimary),
              _chip('${controller.adminCount}',  'Admins', const Color(0xFFF472B6)),
              _chip('${controller.staffCount}',  'Staff',  AppColors.info),
              _chip('${controller.activeCount}', 'Active', AppColors.success),
            ]),
          )),
          ...
          Obx(() => SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, i) => _StoreCard(data: controller.storeOverview[i]),
            ),
          )),
          ...
          Expanded(
            child: Obx(() {
              ...
              if (list.isEmpty) {
                return const Center(
                  child: Text('No staff found', style: TextStyle(color: AppColors.textMuted)),
                );
              }
              return ListView.builder(
                padding: const EdgeInsets.fromLTRB(14, 0, 14, 80),
                itemCount: list.length,
                itemBuilder: (_, i) => _StaffCard(staff: list[i], ctrl: controller),
              );
            }),
          ),
        ],
...
class _StoreCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        const Icon(Icons.store_outlined, color: AppColors.goldPrimary, size: 18),
...
// ════════════════════════════════════════════════════════════════
// STAFF CARD  (with attendance shortcut — matches Electron)
// ════════════════════════════════════════════════════════════════
          ...
          children: [
            ...
            Row(children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    ...
                    const SizedBox(width: 4),
                    Text(staff.status,
                        style: TextStyle(color: isActive ? AppColors.success : AppColors.textMuted, fontSize: 11)),
                  ]),
                ]),
              ),
              // ── Action buttons (matches Electron: view | edit | attendance | toggle) ──
              Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(width: 6),
                // ── Attendance shortcut (new — mirrors Electron clock button) ──
                _iconBtn(Icons.schedule_outlined, AppColors.warning, () {
                  ctrl.selectedStaff.value = staff;
                  // Navigate to staff detail and auto-open Attendance tab
                  Get.toNamed(AppRoutes.staffDetail, arguments: staff.id);
                }, tooltip: 'Attendance'),
                const SizedBox(width: 6),
            ...
            Wrap(
              ...
              runSpacing: 4,
              children: [
                _meta(Icons.store_outlined,          staff.store.replaceAll('Rajmahal Jewellers - ', '')),
                _meta(Icons.phone_outlined,          staff.phone),
                _meta(Icons.currency_rupee_rounded,  '₹${(staff.salary / 1000).toStringAsFixed(0)}K/mo'),
                _meta(Icons.calendar_today_outlined, 'Joined ${_fmtDate(staff.joinDate)}'),
              ],
            ),
            ...
            if (staff.salesTarget > 0) ...[
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                const Text('Sales Target', style: TextStyle(color: AppColors.textMuted, fontSize: 11)),
                Text(
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  valueColor: AlwaysStoppedAnimation(
                    pct >= 80 ? AppColors.success : pct >= 50 ? AppColors.warning : AppColors.error,
                  ),
                  minHeight: 5,
                ),
              ),
            ...
            Wrap(
              ...
              runSpacing: 4,
              children: [
                const Icon(Icons.shield_outlined, color: AppColors.textMuted, size: 12),
                Text(
                ...
                ...staff.permissions.take(4).map((p) {
                  final def = kAllPermissions.firstWhereOrNull((d) => d.id == p);
                ...
                if (staff.permissions.length > 4)
                  Container(
                    ...
                    child: Text('+${staff.permissions.length - 4}',
