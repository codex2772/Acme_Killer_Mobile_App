...
class _EditInventoryScreenState extends State<EditInventoryScreen> {
  ...
  void initState() {
    ...
    // Normalize: if value from backend isn't in the list, fall back to last item 'Other'
    _category = _categories.contains(_item.category) ? _item.category : 'Other';
    _metal = _metals.contains(_item.metal) ? _item.metal : 'Other';
    _purity = _purities.contains(_item.purity) ? _item.purity : 'Other';
    _status = _statuses.contains(_item.status) ? _item.status : 'In Stock';
  }
  ...
  Widget _numField(TextEditingController ctrl, String label) => _wrap(
    label,
    TextFormField(
      controller: ctrl,
