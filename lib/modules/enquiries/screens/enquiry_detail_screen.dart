...
class _EnquiryDetailScreenState extends State<EnquiryDetailScreen> {
    ...
    if (_enquiry == null) {
      return Scaffold(
        ...
        body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const Text('Enquiry not found', style: TextStyle(color: AppColors.textMuted)),
      ...
      body: ListView(padding: const EdgeInsets.all(14), children: [
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(enq.subject,
              ...
              Container(
                ...
                child: Text(enq.status, style: TextStyle(color: statusColor,
                    fontSize: 12, fontWeight: FontWeight.w600)),
              ),
            ]),
            const SizedBox(height: 4),
            Text('${enq.date}${enq.time.isNotEmpty ? " at ${enq.time}" : ""}',
            ...
            Container(
              ...
              child: Text(enq.message.isEmpty ? 'No message provided.' : enq.message,
            ...
            if (enq.hasImage) ...[
              const SizedBox(height: 12),
              const Row(children: [
                Icon(Icons.image_outlined, color: AppColors.textMuted, size: 14),
                SizedBox(width: 6),
                Text('Attached Image', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
              ]),
              const SizedBox(height: 8),
              ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: Image.network(
                  enq.imageUrl!, fit: BoxFit.cover, width: double.infinity,
                  errorBuilder: (_, __, ___) => Container(
                    height: 80, color: AppColors.bgSecondary,
        ...
        if (enq.hasResponse) ...[
          _card('Your Response', Icons.reply_outlined, AppColors.goldPrimary,
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  border: Border(left: BorderSide(color: AppColors.goldPrimary, width: 2.5)),
                ),
                child: Text(enq.adminResponse!,
                    style: const TextStyle(color: AppColors.textSecondary,
                        fontSize: 13, height: 1.5)),
              ),
              if (enq.respondedAt != null) ...[
                const SizedBox(height: 6),
                Text('Responded ${enq.respondedAt!.substring(0, 10)}',
        ...
        if (enq.isOpen && !enq.hasResponse) ...[
          _card('Write Response', Icons.forum_outlined, AppColors.info,
            Column(children: [
              Container(
                decoration: BoxDecoration(color: AppColors.inputFill,
                ...
                child: TextField(
                  controller: _responseCtrl, maxLines: 4,
              ...
              Row(children: [
                ...
                Expanded(flex: 2, child: ElevatedButton.icon(
                  ...
                  icon: _isSending
                      ...
                      : const Icon(Icons.send_outlined, size: 16, color: Colors.black),
        ...
        // ── Customer Info ────────────────────────────────────────
        _card('Customer Info', Icons.person_outline, AppColors.info,
          Column(children: [
        ...
        if (enq.hasLinkedItem)
          _card('Linked Item', Icons.diamond_outlined, AppColors.goldPrimary,
            Column(children: [
              _infoRow('Item', enq.jewelryItemName!),
              if (enq.jewelryItemSku != null) _infoRow('SKU', enq.jewelryItemSku!),
            ]),
          ),
        ...
        if (enq.customerId != null)
          OutlinedButton.icon(
            onPressed: () {
              Get.toNamed(AppRoutes.customerProfile, arguments: enq.customerId.toString());
            },
  ...
  Future<void> _sendResponse() async {
    ...
    if (text.isEmpty) {
      Get.snackbar('Error', 'Please write a response',
  ...
  Future<void> _closeEnquiry() async {
    final confirmed = await Get.dialog<bool>(AlertDialog(
      backgroundColor: AppColors.bgCard,
      title: const Text('Close Enquiry', style: TextStyle(color: AppColors.textPrimary)),
      content: const Text('Are you sure you want to close this enquiry?',
  ...
  // ── Shared helpers ───────────────────────────────────────────────
  Widget _card(String title, IconData icon, Color accent, Widget child) => Container(
    padding: const EdgeInsets.all(16),
    decoration: BoxDecoration(color: AppColors.bgCard,
        borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.border)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
