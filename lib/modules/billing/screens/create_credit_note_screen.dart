    ...
    if (!auth.isDemo.value) {
      ...
      final ok = await _ctrl.createCreditNoteViaApi(
        payload: {
          ...
          'customerId':
              int.tryParse(origInvoice?.customerId ?? '') ??
              origInvoice?.customerId ??
              '',
                  ...
                  items: invoices
                      .map(
                        (inv) => DropdownMenuItem(
                          value: inv.id,
                          child: Text(
                            '${inv.id} — ${inv.customer} (${inv.formattedTotal})',
                            style: const TextStyle(
                              color: AppColors.textPrimary,
                              fontSize: 12,
                            ),
            ...
            Column(
              children: [
                _field(
                  _itemCtrl,
                  'Item Description *',
                  hint: 'e.g., 18K Gold Chain (Returned)',
                ),
            ...
            children: [
              ...
              Expanded(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    _isSubmitting ? 'Processing...' : 'Process Credit Note',
                    style: const TextStyle(
                      color: Colors.black,
