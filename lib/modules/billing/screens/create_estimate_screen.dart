...
class CreateEstimateScreen extends GetView<BillingController> {
                      ...
                      children: [
                        Obx(
                          () => DropdownButtonFormField<String>(
                            ...
                            onChanged: (v) {
                              if (v == null) return;
                              final cust = custCtrl.customers.firstWhereOrNull(
                                (c) => c.name == v,
                              );
          ...
          children: [
            Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'Select store for this estimate',
                    style: TextStyle(
                      color: AppColors.textPrimary,
                      fontWeight: FontWeight.w600,
                      fontSize: 13,
  ...
  Future<void> _save(String validUntil, String notes) async {
    ...
    if (!auth.isDemo.value) {
      final lineItems = controller.items
          .map(
            (item) => {
              'name': item.name,
              'weight': item.weight,
      ...
      final ok = await controller.createEstimateViaApi({
        'customerId':
            int.tryParse(controller.customerId.value) ??
            controller.customerId.value,
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(
                  ctrl.text.isEmpty ? 'Select validity date' : ctrl.text,
                  style: TextStyle(
