...
class ReportType {
  ...
  const ReportType({required this.id, required this.title, required this.desc, required this.iconKey, required this.colorValue});
}
...
class ReportsController extends GetxController {
  ...
  ReportType? get selectedReport {
    ...
    try { return reportTypes.firstWhere((r) => r.id == selectedReportId.value); } catch (_) { return null; }
  }
  ...
  Future<void> _fetchStoreComparison() async {
    ...
    for (final s in stores) {
      storeCtx.switchStore(s.id);
      int revenue = 0; int invoiceCount = 0; int customerCount = 0;

      final ir = await billing.listInvoices();
      if (ir.success && ir.data is List) {
        final invoices = ir.data as List;
        invoiceCount = invoices.length;
        revenue = invoices.fold<int>(0, (sum, i) => sum + ((i as Map)['total'] as num? ?? 0).toInt());
      }
      final cr = await customers.list();
      if (cr.success && cr.data is List) customerCount = (cr.data as List).length;

      result.add({'name': s.name, 'short': s.shortName, 'invoices': invoiceCount, 'revenue': revenue, 'customers': customerCount, 'avg': invoiceCount > 0 ? revenue ~/ invoiceCount : 0});
    }
    storeCtx.clearStore();
  ...
  static const custMonthly = [
    ...
    {'month':'Jan','count':4},{'month':'Feb','count':2},{'month':'Mar','count':3},
  ];
