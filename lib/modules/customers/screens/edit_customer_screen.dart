    ...
    Get.dialog(
      AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              // mirrors Electron: customers.delete(backendId) → API first
              if (_customer.backendId != null) {
                await _ctrl.deleteViaApi(_customer.backendId);
              }
              _ctrl.deleteCustomer(_customer.id);
              Get.until((r) => r.settings.name == '/customers');
              Get.snackbar(
                'Deleted',
                '"${_customer.name}" removed',
                backgroundColor: AppColors.bgCard,
                colorText: AppColors.textPrimary,
                snackPosition: SnackPosition.BOTTOM,
                margin: const EdgeInsets.all(12),
